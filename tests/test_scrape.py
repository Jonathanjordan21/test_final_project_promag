...
class TestScrape(unittest.TestCase):
    ...
    def test_scrape(self):
        data = scrape.run(self.produk, self.jumlah_halaman)
        self.assertGreater(len(data), 6)
    def test_scrape_review(self):
        data = scrape_review.run(self.link, self.jumlah_halaman)
        self.assertGreater(len(data), 6)
    def test_scrape_toko(self):
        data = scrape_toko.run(self.nama_toko, self.jumlah_halaman)
        self.assertGreater(len(data), 6)
    def test_scrape_review_toko(self):
        data = scrape_review_toko.run(self.nama_toko, self.jumlah_halaman)
        self.assertGreater(len(data), 6)
